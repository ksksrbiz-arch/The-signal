// Shared site chrome (nav, header, footer) for generator scripts.
//
// `depth` is how many directory levels the generated page sits below the site
// root: 1 for /playbooks/foo.html or /archive/019.html, 0 for a root page. The
// prefix keeps relative links correct without hardcoding a path per template.
...
function hrefFor(target, depth) {
  return `${prefixFor(depth)}${target}`;
}

/**
 * @param {number} depth  directory levels below site root
 * @param {string} active label of the nav item to mark as current, if any
 */
export function siteHeader(depth, active = '') {
  const navLinks = NAV_ITEMS.map(([label, target]) =>
    label === active
      ? `            <li><a href="${hrefFor(target, depth)}" class="active" aria-current="page">${label}</a></li>`
      : `            <li><a href="${hrefFor(target, depth)}">${label}</a></li>`,
  ).join('\n');

  const mobileLinks = NAV_ITEMS.map(([label, target]) =>
    label === active
      ? `    <a href="${hrefFor(target, depth)}" class="active" aria-current="page">${label}</a>`
      : `    <a href="${hrefFor(target, depth)}">${label}</a>`,
  ).join('\n');
...
${mobileLinks}
    <a href="${prefixFor(depth)}#subscribe">Subscribe</a>
  </div>

  <header class="site-header">
    <div class="header-inner">
      <div class="header-left">
        <a href="${prefixFor(depth)}">
...
export function siteFooter(depth) {
  const footerNav = NAV_ITEMS.map(
    ([label, target]) => `          <li><a href="${hrefFor(target, depth)}">${label}</a></li>`,
  ).join('\n');
...
${footerNav}
    ...
    <div class="footer-bottom">
      <div class="footer-bottom-left">
        <span class="mono-label">© ${new Date().getUTCFullYear()} 1COMMERCE LLC · CANBY, OREGON</span>
